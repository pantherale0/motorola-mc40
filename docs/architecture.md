...
| `CompanionService` | Foreground service: sensors, notify socket, scan/button/proximity publish, beep/vibrate/LED |
...
WebSocket `event` payload is parsed on the socket thread. Recognised `data.command` values are posted on `OverlayBus` (main thread). Unknown notifies become a Toast.
